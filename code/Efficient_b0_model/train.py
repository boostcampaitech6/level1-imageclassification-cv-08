...
import wandb
wandb.login()
wandb.init(project='1221_cutmix_effi_gender_model')
import time
start=time.time()
...
gender=pd.read_csv('/home/kangdg22/meta_Assignment/boostcamp/my_code/dataset/gender.csv')
...
def train(model, optimizer, train_loader, test_loader, scheduler, device, k_idx, beta=1, cut_mix=0.3):
    ...
    for epoch in range(1, CFG['EPOCHS']+1):
        ...
        if best_score<val_score:
            ...
            torch.save(best_model.state_dict(), f'/home/kangdg22/meta_Assignment/boostcamp/my_code/soft_voting/cutmix_gender/fold_best_model{k_idx+1}.pt')
...
for fold, (train_index, val_index) in enumerate(skf.split(gender['id'], gender['label'])):
    age_train_dataset=CustomDataset(gender['id'].iloc[train_index].values, gender['label'].iloc[train_index].values, transform=train_transform)
    age_train_loader=DataLoader(age_train_dataset, batch_size=CFG['BATCH_SIZE'], shuffle=True, num_workers=0)

    age_val_dataset=CustomDataset(gender['id'].iloc[val_index].values, gender['label'].iloc[val_index].values, transform=test_transform)
...
weight_save_path=[f'/home/kangdg22/meta_Assignment/boostcamp/my_code/soft_voting/cutmix_gender/fold_best_model{i}.pt' for i in range(1,6)]
...
df_gender=pd.DataFrame({'ImageID':test_data['ImageID'],'gender':pred})


# In[18]:


df_gender.to_csv('/home/kangdg22/meta_Assignment/boostcamp/my_code/df_cutmix_gender.csv', index=False)
