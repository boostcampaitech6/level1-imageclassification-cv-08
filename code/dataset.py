...
class MaskBaseDataset(Dataset):
    ...
    def split_dataset(self, n_splits=5):
        """데이터셋을 StratifiedKFold를 사용하여 나누는 메서드"""
        skf = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=42)

        multi_class_labels = [
            self.encode_multi_class(m, g, a)
            for m, g, a in zip(self.mask_labels, self.gender_labels, self.age_labels)
        ]
        subsets = []

        for train_index, val_index in skf.split(self.image_paths, multi_class_labels):
            train_subset = Subset(self, train_index)
            val_subset = Subset(self, val_index)
